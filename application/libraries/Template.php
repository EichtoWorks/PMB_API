...
class Template
{
	...
	private $_footer_metadata = array();
	...
	function initialize($config = array())
	{
		...
		if ($this->_theme_locations === array())
		{
			...
			$this->_theme_locations = array(ADDONS . 'themes/');
		}
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['footer_metadata']	= implode("\n\t\t", $this->_footer_metadata);
	...
	public function footer_metadata($line)
	{
		$this->_footer_metadata[] = $line;
		return $this;
	}
	
	public function append_css($files)
	{
		$matches = preg_split("/[\/,]+/", $files);
		
		if($matches){
			$ext = explode(".", $matches[1]);
			
			if( $ext[1] === 'css' && $matches[0] === $this->_module){
				$css = '<link rel="stylesheet" type="text/css" href="'.MODULE_PATH. $matches[0]. '/css/'. $matches[1] .'" media="all">';
			}
			/*else{
				$css = '<link rel="stylesheet" type="text/css" href="'.MODULE_PATH. $matches[0]. '/css/'. $matches[1] .'" media="all">';
			}*/
	...
	public function append_js($files)
	{
		$matches = preg_split("/[\/,]+/", $files);
		
		if($matches){
			$ext = explode(".", $matches[1]);
			if( $ext[1] === 'js' && $matches[0] === $this->_module){
				$js = '<script type="text/javascript" src="'.MODULE_PATH. $matches[0]. '/js/'. $matches[1] .'"></script>';
			}
			/*else{
				$css = '<link rel="stylesheet" type="text/css" href="'.MODULE_PATH. $matches[0]. '/css/'. $matches[1] .'" media="all">';
			}*/
